...
##
## @param node: The FlowState node using this behavior.
...
## @param actor: The owner of the state machine.
func update(node: Node, delta: float, actor: Node) -> void:
	pass

## Helper to get the structured memory object for this state.
## Note: This feature relies on FlowState implementation details.
