...
static func is_state(node: Node) -> bool:
	...
	return node is RecursiveState or (node.get_script() and node.get_script().resource_path.ends_with("RecursiveState.gd"))
