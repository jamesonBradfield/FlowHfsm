...
func enter(node: RecursiveState, _actor: Node, _blackboard: Dictionary) -> void:
	node.memory["timer"] = 0.5
	node.is_locked = true

func update(node: RecursiveState, delta: float, _actor: Node, _blackboard: Dictionary) -> void:
