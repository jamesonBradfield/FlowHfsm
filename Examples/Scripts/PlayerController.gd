...
## Strictly SRP: Does NOT calculate game logic (speed, animation state, etc).
...
## Action name for dash.
@export var dash_action: String = "dash"
## Action name for roll.
@export var roll_action: String = "roll"

# -- Public State Properties --
# Accessed directly by Smart Values (BehaviorPhysics, ConditionInput)
...
var roll_just_pressed: bool = false

# Computed property for AnimationDrivers
var current_speed: float:
	get: return _body.velocity.length() if _body else 0.0

var is_grounded: bool:
	get: return _body.is_on_floor() if _body else false
...
func _poll_input() -> void:
	...
	# Poll Dash/Roll
	dash_just_pressed = Input.is_action_just_pressed(dash_action)
	roll_just_pressed = Input.is_action_just_pressed(roll_action)
